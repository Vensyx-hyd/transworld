...
class HireTrailerReports extends React.Component {
  ...
  getHireTrailers() {
    AppAPI.hireTrailReport.get(null, null).then((resp) => {
      console.log(resp.hireTrailer, "Get Hire Trailer");
      this.setState({
        rows: resp.hireTrailer,
        getApi: true,
        total: resp.hireTrailer.length
      })
  ...
  handleInputSearch(event) {
    ...
    let  sortByTrailerNo = this.trailerReport();
  ...
  getDateRange(from, to) { 
    if (from && to !== "") {
      ...
      AppAPI.hireTrailReport.get(('?from='+from+'&to='+to), null).then((resp) => {
        console.log(resp.hireTrailer, "Get Date Range");
        this.setState({
          items: resp.hireTrailer,
          reItems: resp.hireTrailer,
          filter: true,
          total: resp.hireTrailer.length,
          pageNo: resp.hireTrailer.length,
          apply: true,
          submit: false
        })
      }).catch((err) => console.log(err))
  ...
  render(){
    ...
    const grid =(      
      ...
      >        
        ...
        <GridColumn field='hireDate' title='Date' width='150'/>
        <GridColumn field='trip_type' title='Cycle' width='150'/>
        <GridColumn field='trailerNo' title='Trailer No.' width='150'/>
        <GridColumn field='container_no' title='Container No.1' width='150'/>
        <GridColumn field='container_no2' title='Container No.2' width='150'/>
        <GridColumn field='size' title='Size' width='100' />    
        {/* <GridColumn field='' title='Type' width='100' />     */}
        <GridColumn field='loaded_or_empty' title='Trip Type' width='250' />      
        <GridColumn field='rate' title='Per Trip Cost' width='150' />  
      </Grid>
    );
    return(   
      getApi === false
      ...
      :
      <div>
         <p className="titleCard">Reports / Operational / Hire Trailer</p>
...
HireTrailerReports.propTypes = {
  classes: PropTypes.object.isRequired,
};
  
export default withStyles(tableStyles)(HireTrailerReports);