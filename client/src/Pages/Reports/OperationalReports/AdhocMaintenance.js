...
class AdhocMaintenance extends React.Component {
  ...
  getAdhocMain() {
    AppAPI.adhocMainReport.get(null, null).then((resp) => {
      console.log(resp.adhocMain, "Get Adhoc");
      this.setState({
        rows: resp.adhocMain,
        getApi: true,
        total: resp.adhocMain.length
  ...
  getDateRange(from, to) { 
    if (from && to !== "") {
      ...
      AppAPI.adhocMainReport.get(('?from='+from+'&to='+to), null).then((resp) => {
        console.log(resp.adhocMain, "Get Date Range");
        this.setState({
          items: resp.adhocMain,
          reItems: resp.adhocMain,
          filter: true,
          total: resp.adhocMain.length,
          pageNo:resp.adhocMain.length,
          apply: true,
          submit: false
        })
      }).catch((err) => console.log(err))
  ...
  render(){
    ...
    const grid =(      
      ...
      >        
        ...
        <GridColumn field='createdDate' title='Date' width='200'/>
        <GridColumn field='trailerNo' title='Trailer No.' width='250'/>
        <GridColumn field='requestedServiceType' title='Requested Service Type' width='250'/>
        <GridColumn field='driverName' title='Driver Name'/>
        {/* <GridColumn field='' title='Remark'/>         */}
      </Grid>
    );
    return(   
      getApi === false
      ...
      :
      <div>
         <p className="titleCard">Reports / Operational / Adhoc Maintenance</p>
...
AdhocMaintenance.propTypes = {
  classes: PropTypes.object.isRequired,
};
  
export default withStyles(tableStyles)(AdhocMaintenance);