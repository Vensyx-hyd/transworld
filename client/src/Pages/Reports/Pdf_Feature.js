...
class Driver extends React.Component {
  ...
  exportPDF = () => {
    ...
    this.gridPDFExport.save(data, this.lowerPDFExportRequestedFlag);  
  }
  ...
  render(){
    const { getApi, rows, pdf } = this.state;
      ...
      >
        ...
        <div className="row float-right">
            <Paper className={classes.barRoot} elevation={1}>
              ...
              <IconButton title="Export PDF" color="primary" className={classes.barIconButton} 
                aria-label="Directions" onClick={this.exportPDF} disabled={this.state.pdfExportRequested}>
                {
                  this.state.pdf === 1
                  ?
                  <Spinner/>
                  :
                  <img src="/assets/icons/pdf.svg" className="selectionControlPdf" alt="Pdf"/>
                }                     
              </IconButton>
            </Paper>
        </div> 
