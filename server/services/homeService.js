...
class HomeService {
  ...
  async getDrivers() {
    ...
    return await this.db.query("select ceo_dr_shift as on_trip, ceo_dr_pr as shift1_present, ceo_dr_abs as shift1_absent, ceo_driver_shift2_absent as shift2_absent, ceo_dr_leave as shift1_leave, ceo_driver_shift2_leave as shift2_leave, ceo_dr_pay as on_pay , ceo_driver_shift1 as shift1_cnt, ceo_driver_shift2 as shift2_cnt, ADDTIME(ceo_dr_dt, '5:30') as date from ceo_driver_report"); // ceo_dr_pr_shift2 as shift2_present,
  ...
  async getTransitList() {
    var result = await this.db.query("select mob_user_notify_id, mob_notify_type_id, mob_login_key, Driv_name from cfs_dri_msg where substr(Cfs_sri_msg_cr_dt,1,10) = (substr(now(),1,10)) AND msg_removing_status is null");
    console.log(result);

    var result1 = await this.db.query("select mob_notify_type_desc from mob_notify_type where mob_notify_type_id = ?", [result[0].mob_notify_type_id]);
    // console.log(result1);

    var result2 = await this.db.query("select mob_user_notify_id, user_ass_trip_id, mob_notify_type_id, mob_login_key,created_loc from mob_user_notify where mob_user_notify_id = ?", [result[0].mob_user_notify_id]);
    // console.log(result2);

    var result3 = await this.db.query("select user_trip_id, system_user_id, trailor_no, trip_status from user_assign_trip where user_ass_trip_id = ?", [result2[0].user_ass_trip_id]);
    // console.log(result3);

    var result4 = await this.db.query("select start_loc, end_loc, exp_start_time, exp_end_time from user_trips where user_trip_id = ?", [result3[0].user_trip_id]);
    // console.log(result4);

    const [latitude, longitude] = result4[0].start_loc.split(',');
    const [latitude1, longitude1] = result4[0].end_loc.split(',');
    var result5 = await this.db.query("select loc_name, loc_long, loc_lati from location where (loc_lati = ? AND loc_long = ?) OR (loc_lati = ? AND loc_long = ?)", [latitude, longitude, latitude1, longitude1]);
    // console.log(result5);

    const labeledResults = result5.map((location) => {
      if (location.loc_lati == latitude && location.loc_long == longitude) {
        return { Start_Location: location.loc_name };
      } else if (location.loc_lati == latitude1 && location.loc_long == longitude1) {
        return { End_Location: location.loc_name };
      }
    });
    ...
    const transitResult = [
      {
        "trip_no": result3[0].user_trip_id,
        "driver_name": result[0].Driv_name,
        "triler_no": result3[0].trailor_no,
        "trip_status": result3[0].trip_status,
        "start_loc": labeledResults[0].Start_Location,
        "end_loc": labeledResults[1].End_Location,
        "est_st_dt_ti": result4[0].exp_start_time,
        "est_en_dt_ti": result4[0].exp_end_time,
        "trip_issue": result1[0].mob_notify_type_desc
      }
    ];
